...
export function TaskFormPage() {
  const { createTask, getTask, updateTask, tasks, getTasks } = useTasks();
  const navigate = useNavigate();
  const params = useParams();
  const {
    register,
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (params.id) {
        ...
        updatedTask = tasks.find((task) => task._id === params.id);
      } else {
        // Si es una creación, esperar a que se complete la operación
        await createTask({
          ...data,
        ...
        await getTasks(); // Actualizar la lista de tareas después de la creación
      }
      ...
      // Puedes hacer más cosas aquí, como mostrar un mensaje al usuario con la tarea creada o actualizada
      console.log(`Tarea ${params.id ? 'actualizada' : 'creada'} con éxito:`, updatedTask || data);
    } catch (error) {
      // Manejar errores si ocurren durante la operación
      console.error('Error al procesar la tarea:', error.message);
