...
import DiagnosticMonitor from "../components/tasks/DiagnosticMonitor.jsx"; // Importa el componente DiagnosticMonitor
...
export function TaskProvider({ children }) {
  ...
  const filterTasks = async () => {
    try {
      const res = await filterTasksRequest(search);
      console.log(res.data);
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    ...
    >
      {/* Agrega el componente DiagnosticMonitor como parte del contexto */}
      <DiagnosticMonitor />
      {children}
    </TaskContext.Provider>
  );
